# Standards: 0.1
# This standard is *heavily* based on https://github.com/willthames/ansible-review/blob/master/lib/ansiblereview/examples/standards.py
import codecs
import os
import yaml
...
# The indentation check doesn't work if the indentation doesn't start at 0.
...
roles_git_scm_version = Standard(dict(
    name="Git checkouts must contain explicit version",
    check=lintcheck('ANSIBLE0004'),
    types=["rolesfile"]
))
...
check_mode_instead_of_always_run = Standard(dict(
    check=lintcheck('ANSIBLE0018'),
    name="Instead of always_run, use check_mode",
    types=["playbook", "task", "handler"]
))

check_whitespace = Standard(dict(
    check=lintcheck('ANSIBLE0002'),
    name="There should not be any trailing whitespace",
    types=["playbook", "task", "handler"]
))

no_jinja_in_when = Standard(dict(
    check=lintcheck('ANSIBLE0019'),
    name="\"when\" lines should not include Jinja2 variables",
    types=["playbook", "task", "handler"]
))
...
ansible_min_version = '2.6.5'
ansible_review_min_version = '0.13.8'
ansible_lint_min_version = '3.5.1'
...
standards = [
    ...
    latest_version,
    check_mode_instead_of_always_run,
    roles_git_scm_version,
    no_jinja_in_when,
    check_whitespace,
